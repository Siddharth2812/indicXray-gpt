...
def process_folder_images(folder_path, output_csv=None):
    ...
    if output_csv is None:
        output_csv = f"image_analysis_results_gemini.csv"
...
def process_images_from_csv(csv_file_path, images_folder_path, concurrent_workers=10, output_csv=None):
    ...
    if 'gemini_analysis' not in df.columns:
        df['gemini_analysis'] = None
    
    # Create output filename with timestamp if not provided
    if output_csv is None:
        output_csv = f"gemini_analysis_results.csv"
    ...
    with ThreadPoolExecutor(max_workers=concurrent_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(futures), total=total_to_process, desc="Processing images"):
            ...
            try:
                ...
                df.at[idx, 'gemini_analysis'] = result['response']
    ...
    # Print summary
    success_count = sum(1 for result in df['gemini_analysis'] if result is not None and not str(result).startswith('Error:'))
